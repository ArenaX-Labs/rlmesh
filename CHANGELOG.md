...
- A decomposable endpoint timing: every env and model response now carries the split of its `endpoint_total_ns` into wire decode, the implementation's own work, and wire encode, and a model response adds the wait before its predict handler ran plus the slots the endpoint had taken. The runtime records them as the `endpoint.decode`, `endpoint.user`, `endpoint.encode`, `predict.queue`, and `predict.in_flight` metrics, so a slow step can be attributed to serialization, user code, or queueing without attaching a profiler. A peer that does not stamp them is unchanged. The phase-summary log moved to `RLMESH_PROFILE_PHASES`; `RLMESH_PROFILE` still enables it when it reads as a boolean, but it primarily names the CLI's credential profile.
...
- Straggler visibility for vector environments: an env that times its own lanes stamps one `lane_skew_ns` scalar per op — its slowest lane minus its median lane — which the runtime records as the `lane.skew` metric. One pathologically slow lane among 32 shows up in that metric's p95/p99 instead of merely blurring the `env.step` aggregate, and it stays one series no matter how many lanes the env runs. Compute it with `rlmesh::env::lane_skew_ns` over a scratch buffer of per-lane durations.
