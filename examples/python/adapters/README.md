...
The smallest end-to-end `rlmesh.adapters` example: an environment tags its spaces, a model declares its format, and the pairing is derived at runtime with no per-environment glue in the model.
...
2. `EnvServer(env, tags=ENV_TAGS)` serves it and publishes the tags in the contract (validated against the env's spaces first).
