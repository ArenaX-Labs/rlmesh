...
This example serves `sai_pygame:SquidHunt-v0` through RLMesh and reuses the shared quickstart client
for evaluation.

This folder intentionally has its own `pyproject.toml`, `uv.lock`, and mise-managed `.venv` so
`sai-pygame` dependencies stay out of the root development environment.
