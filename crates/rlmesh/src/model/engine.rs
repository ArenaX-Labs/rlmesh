...
/// Validate the observation and assemble each
/// lane's model input. Runs under the route's entry lock (frame-stacking
/// mutates the per-episode buffers here, once per call).
///
...
/// Dispatch to the most-specific available
/// corner, yielding each lane's raw chunk frames (a single-element Vec when not
/// chunking). Prefer a *batched* corner (one forward for the whole vector) over
/// the per-lane loop, and a *chunk* corner when the runtime pinned a
/// horizon > 1. A horizon > 1 with no chunk corner was already warned at
/// configure and falls through to a single-action corner (the runtime then
/// re-plans every step). split_chunk caps each chunk to the horizon (a
/// receding-horizon model may over-produce).
...
/// Apply the per-step adapter transform to each
/// frame, peel frame 0 (this step's action) from the future frames (which the
/// runtime replays), and transpose the per-lane future frames into per-step
/// batched frames (`replay[step][lane]`), to the shortest lane (uniform for a
/// homogeneous fleet; a short lane caps the batch — receding horizon).
...
/// gate guarantees at least one batched corner is present.
...
fn predict_grouped_fused(
    ...
    for (entry, observation) in entries.iter().zip(observations) {
        match entry {
            // A spec-less route has no adapter or chunk semantics; serve it
            // through the preserved raw path, matching `predict_chunked`.
            None => slots.push(Slot::Done(predict.predict_spec_less(observation).map(
                |actions| PredictFrames {
                    actions,
                    replay: Vec::new(),
                },
            ))),
...
impl ModelHandler for AdaptedModelHandler {
    ...
    async fn predict_grouped(
        ...
        tokio::task::spawn_blocking(move || predict_grouped_fused(entries, observations, &predict))
            .await
            .unwrap_or_else(|err| {
