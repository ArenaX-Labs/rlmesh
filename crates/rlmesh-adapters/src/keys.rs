...
//!
//! The version segment of these key strings — not any source-module path — is
//! the adapter spec-format discriminator. A breaking spec-format change ships a
//! v2 key (`rlmesh.adapters.v2.*`); a v2 reader MUST also keep reading the v1
//! key (dual-read) so a newer build still resolves an older peer's specs.
