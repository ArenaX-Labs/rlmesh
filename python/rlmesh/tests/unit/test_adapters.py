...
def _chunked_spec(execute_horizon: int, *, stack: int = 1) -> adapt.ModelSpec:
    """A minimal single-image -> 7-dim-action spec with action-chunk replay."""
    return adapt.ModelSpec(
        inputs=(
            adapt.ImageInput(
                "image", role=adapt.IMAGE_PRIMARY, height=64, width=64, stack=stack
            ),
        ),
        action=adapt.ActionLayout(
            adapt.ActionComponent(adapt.ACTION_DELTA_POS, dim=3),
...
def test_wrap_predict_replays_an_action_chunk_within_an_episode():
    # The explicit adapter.wrap_predict path must replay a chunk like Model.run:
    # a [2, 7] chunk replays row 1 on the next step instead of passing the whole
    # chunk into transform_action (which would fail the width check). predict_fn
    # fires once per horizon, and reset() drops any un-replayed tail.
    adapter = resolve(LIBERO_ENV, _chunked_spec(2))
    obs = make_obs()
    calls = {"n": 0}

    def predict_fn(payload: dict[str, Any]) -> Any:
        calls["n"] += 1
        c = calls["n"]
        # Rows distinct within and across calls, all inside the [-1, 1] clip.
        return np.stack(
            [
                np.full(7, 0.1 * (2 * c - 1), dtype=np.float32),
                np.full(7, 0.1 * (2 * c), dtype=np.float32),
            ]
        )

    wrapped = adapter.wrap_predict(predict_fn)
    a0 = wrapped(obs)  # predict -> chunk row 0
    a1 = wrapped(obs)  # replay  -> chunk row 1 (predict_fn NOT called)
    a2 = wrapped(obs)  # re-plan -> call 2, row 0

    assert calls["n"] == 2, calls
    assert a0.shape == (7,)
    assert not np.array_equal(a0, a1), "step 1 did not replay row 1"
    assert not np.array_equal(a1, a2), "step 2 did not re-plan"
    assert not np.array_equal(a0, a2)

    # reset() drops the un-replayed tail (call-2 row 1 still queued), so the next
    # call re-plans rather than replaying a stale action from the prior episode.
    adapter.reset()
    calls["n"] = 0
    wrapped(obs)
    assert calls["n"] == 1, "reset did not drop the queued chunk tail"


def test_frame_stacking_combined_with_chunk_horizon_is_rejected():
    # stack>1 + execute_horizon>1: during chunk replay the engine skips obs
    # assembly, so a stacked input would only ever see decision-point frames.
    # Rejected at resolve (the one seam both serve and run(env) resolve through).
    with pytest.raises(adapt.AdapterResolutionError, match="frame-stacking") as info:
        resolve(LIBERO_ENV, _chunked_spec(2, stack=2))
    assert "execute_horizon" in str(info.value), info.value
