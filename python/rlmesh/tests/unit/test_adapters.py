...
# ---------------------------------------------------------------------------
# Shared LIBERO-style env spec and a synthetic observation.
# ---------------------------------------------------------------------------
...
def test_smolvla_obs_matches_bespoke_adapter():
    obs = make_obs()
    adapter = adapt.resolve(LIBERO_ENV, SMOLVLA)
...
def test_openvla_obs_matches_bespoke_adapter():
    obs = make_obs()
    adapter = adapt.resolve(LIBERO_ENV, OPENVLA)
    payload = adapter.transform_obs(obs)
...
def test_xvla_state_matches_bespoke_adapter():
    obs = make_obs()
    adapter = adapt.resolve(LIBERO_ENV, XVLA)
    payload = adapter.transform_obs(obs)
...
def test_xvla_action_matches_bespoke_adapter():
    obs_adapter = adapt.resolve(LIBERO_ENV, XVLA)
...
BIMANUAL_ENV = adapt.EnvIOSpec(
    observation=(
        ...
        adapt.EnvState("robot0_eef_quat", role=adapt.EEF_ROT, encoding="quat_xyzw"),
        adapt.EnvState("robot0_gripper_qpos", role=adapt.GRIPPER_POS, dim=2),
        adapt.EnvState("robot1_eef_pos", role=adapt.EEF_POS_2, dim=3),
        adapt.EnvState("robot1_eef_quat", role=adapt.EEF_ROT_2, encoding="quat_xyzw"),
        adapt.EnvState("robot1_gripper_qpos", role=adapt.GRIPPER_POS_2, dim=2),
        adapt.EnvText("instruction"),
    ),
    action=adapt.ActionLayout(
        components=(
...
def test_xvla_state_consumes_second_arm_on_bimanual_env():
    ...
    adapter = adapt.resolve(BIMANUAL_ENV, XVLA)
    payload = adapter.transform_obs(obs)
...
def test_optional_state_without_width_is_an_error():
    ...
    with pytest.raises(adapt.AdapterResolutionError, match="zero fill"):
        adapt.resolve(LIBERO_ENV, spec)


def test_describe_mentions_zero_fill_for_absent_optional_roles():
    text = adapt.resolve(LIBERO_ENV, XVLA).describe()
...
def test_custom_adapter_subclass_is_interchangeable():
    ...
    assert isinstance(adapt.resolve(LIBERO_ENV, SMOLVLA), adapt.AdapterBase)
...
def test_gr00t_obs_matches_bespoke_adapter():
    obs = make_obs()
    adapter = adapt.resolve(LIBERO_ENV, GR00T)
    payload = adapter.transform_obs(obs)
...
def test_image_resize_layout_and_normalize():
    ...
    payload = adapt.resolve(LIBERO_ENV, spec).transform_obs(obs)
...
def test_bilinear_aa_resize_matches_pillow_within_one_step():
    ...
    env = adapt.EnvIOSpec(
        observation=(adapt.EnvImage("rgb"),),
        action=LIBERO_ACTION,
    )
    image = (
    ...
    for height, width in ((3, 4), (12, 16)):
        ...
        ours = (
            adapt.resolve(env, spec)
            .transform_obs({"rgb": image})["image"]
            .astype(np.int16)
        )
        theirs = np.asarray(
...
def test_encoded_image_bytes_decode_natively():
    ...
    payload = adapt.resolve(env, spec).transform_obs({"rgb": make_png(pixels)})
...
def test_undecodable_image_bytes_is_an_error():
    ...
    with pytest.raises(ValueError, match="could not decode image bytes"):
        adapt.resolve(env, spec).transform_obs({"rgb": b"not an image"})
...
def test_bilinear_resize_preserves_constant_images():
    ...
    payload = adapt.resolve(env, spec).transform_obs(
...
def test_single_env_image_fallback_match():
    env = adapt.EnvIOSpec(
        observation=(
            adapt.EnvImage("rgb", role=adapt.IMAGE_SECONDARY),
            adapt.EnvText("instruction"),
        ),
        action=LIBERO_ACTION,
    )
    ...
    adapter = adapt.resolve(env, spec)
    obs = {"rgb": np.zeros((8, 8, 3), dtype=np.uint8)}
    assert adapter.transform_obs(obs)["image"].shape == (8, 8, 3)


# ---------------------------------------------------------------------------
# Lookup, custom transforms, wrap_predict
# ---------------------------------------------------------------------------


def test_nested_observation_keys():
    env = adapt.EnvIOSpec(
        observation=(adapt.EnvState("agent.eef_pos", role=adapt.EEF_POS, dim=3),),
        action=LIBERO_ACTION,
    )
    spec = adapt.ModelIOSpec(
        inputs=(
            adapt.StateInput(
                "state", components=(adapt.StateComponent(adapt.EEF_POS),)
            ),
        ),
        action=SMOLVLA.action,
    )
...
def test_numeric_payload_data_mapping():
    env = adapt.EnvIOSpec(
        observation=(adapt.EnvState("pos", role=adapt.EEF_POS, dim=3),),
        action=LIBERO_ACTION,
    )
    spec = adapt.ModelIOSpec(
        inputs=(
...
def test_custom_callable_transform():
    ...
    adapter = adapt.resolve(LIBERO_ENV, spec)
    obs = make_obs()
...
def test_custom_entrypoint_requires_trust():
    ...
    adapter = adapt.resolve(LIBERO_ENV, spec, trust_entrypoints=True)
...
def test_env_spec_json_round_trip():
    assert adapt.EnvIOSpec.from_json(LIBERO_ENV.to_json()) == LIBERO_ENV


def test_model_spec_json_round_trip():
    for spec in (SMOLVLA, OPENVLA, XVLA, GR00T):
        assert adapt.ModelIOSpec.from_json(spec.to_json()) == spec


def test_env_spec_metadata_round_trip():
    metadata = {"render_fps": 20, **LIBERO_ENV.to_metadata()}
    assert adapt.EnvIOSpec.from_metadata(metadata) == LIBERO_ENV
    assert adapt.EnvIOSpec.from_metadata({"render_fps": 20}) is None


def test_model_spec_metadata_round_trip():
    metadata = {"max_batch": 8, **SMOLVLA.to_metadata()}
    assert adapt.ModelIOSpec.from_metadata(metadata) == SMOLVLA
    assert adapt.ModelIOSpec.from_metadata({"max_batch": 8}) is None


def test_metadata_keys_are_side_specific():
    assert adapt.ENV_METADATA_KEY != adapt.MODEL_METADATA_KEY
    merged = {**LIBERO_ENV.to_metadata(), **SMOLVLA.to_metadata()}
    assert adapt.EnvIOSpec.from_metadata(merged) == LIBERO_ENV
    assert adapt.ModelIOSpec.from_metadata(merged) == SMOLVLA
    assert adapt.EnvIOSpec.from_metadata(SMOLVLA.to_metadata()) is None
    assert adapt.ModelIOSpec.from_metadata(LIBERO_ENV.to_metadata()) is None
...
def test_state_rotation_conversion_width_mismatch_is_an_error():
    env = adapt.EnvIOSpec(
        observation=(
            adapt.EnvImage("agentview_image"),
            adapt.EnvState(
                "robot0_eef_quat", role=adapt.EEF_ROT, encoding="quat_xyzw", dim=5
            ),
            adapt.EnvText("instruction"),
        ),
        action=LIBERO_ACTION,
    ...
    with pytest.raises(adapt.AdapterResolutionError, match="declares 5 dims"):
        adapt.resolve(env, spec)


def test_state_same_encoding_width_is_not_enforced():
    # No conversion is scheduled, so the value is transported as-is and
    # ROTATION_DIMS has no precondition to enforce.
    env = adapt.EnvIOSpec(
        observation=(
            adapt.EnvImage("agentview_image"),
            adapt.EnvState(
    ...
    spec = adapt.ModelIOSpec(
        inputs=(
            adapt.StateInput(
                "state",
                components=(adapt.StateComponent(adapt.EEF_ROT, encoding="quat_xyzw"),),
            ),
        ),
        action=SMOLVLA.action,
    )
