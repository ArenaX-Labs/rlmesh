...
The native core speaks a small tagged-tuple encoding (see the
``adapters`` module of the ``rlmesh-python`` crate): arrays travel as
``("a", dtype, shape, bytes)``, encoded PNG/JPEG images as ``("b",
bytes)`` (decoded to RGB uint8 arrays on the native side), text as
``("t", str)``, numbers as ``("n", float)``, lists as ``("l", [...])``,
and nested mappings as ``("m", {...})``. PIL image objects are converted
to arrays here, since holding one means PIL is already loaded in this
process; nothing in this module imports it.
"""
