"""Internal value conversion: the value-tree codec plus the per-framework bridge.

Each framework module (``rlmesh.numpy``, ``rlmesh.torch``, ``rlmesh.jax``)
supplies three leaf callables and gets uniform tree-walking value conversion via
:class:`FrameworkBridge`; everything else in those modules is framework-specific
