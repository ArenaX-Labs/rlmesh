...
- package an environment or model as a container image and run it the same way locally or on the managed platform (see the {doc}`bring-your-own-container example <examples/byo-container>`).

RLMesh has two surfaces, split by protocol. The framework is this repository. It speaks the gRPC runtime contract, so you author and run environments and models in Python, on one machine or over the wire. The managed platform speaks a REST control plane at `api.rlmesh.dev` and is documented separately. These docs cover the framework.
