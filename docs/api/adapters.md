...
```{note}
This is the autodoc API reference. For the guided tour see {doc}`../user-guide/adapters`; for the
full role registry, every leaf option, and the conversion policy see {doc}`../user-guide/adapters/reference`.
```
...
{attr}`EnvTags.observation_roles <rlmesh.adapters.EnvTags.observation_roles>` (and `Session.observation_roles` on a live session) groups the declared roles by kind:

```{eval-rst}
.. autoclass:: rlmesh.adapters.ObservationRoles
   :class-doc-from: class
   :exclude-members: __init__, __new__
```
